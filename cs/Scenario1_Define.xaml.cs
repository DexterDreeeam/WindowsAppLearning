...
namespace SDKTemplate
{
    public sealed partial class Scenario1_Define : Page
    {
        ...
        private void Test(Appointment app)
        {
            Test2(app);
        }

        private async Task Test1(Appointment app)
        {
            var store = await AppointmentManager.RequestStoreAsync(AppointmentStoreAccessType.AppCalendarsReadWrite);
            //var appCal = await store.CreateAppointmentCalendarAsync("Test123");
            var allAppCals = await store.FindAppointmentCalendarsAsync();
            var size = allAppCals.Count;
            foreach (var it in allAppCals)
            {
                DateTimeOffset rangeStart = DateTimeOffset.Now.AddYears(-1);
                TimeSpan rangeLength = TimeSpan.FromDays(2 * 365);
                var name = it.DisplayName;
                var apps = await it.FindAppointmentsAsync(rangeStart, rangeLength);
        ...
        private async Task Test2(Appointment app)
        {
            var store = await AppointmentManager.RequestStoreAsync(AppointmentStoreAccessType.AppCalendarsReadWrite);
            var appCal = await store.CreateAppointmentCalendarAsync("TestE");
            appCal.OtherAppReadAccess = AppointmentCalendarOtherAppReadAccess.None;
            appCal.OtherAppWriteAccess = AppointmentCalendarOtherAppWriteAccess.None;
            await appCal.SaveAppointmentAsync(app);
